...
int main()
{
    ...
    if (!route->NewLink(rate, lark::SampleFormat::FLOAT_LE, chNum, frameSizeInSamples, blkFormatAdapter, 0, blkSoundTouch, 0)) {
        KLOGE("Failed to new a link");
        return -1;
    }
    if (!route->NewLink(rate, lark::SampleFormat::FLOAT_LE, chNum, frameSizeInSamples, blkSoundTouch, 0, blkPAPlayback, 0)) {
        KLOGE("Failed to new a link");
        return -1;
    }
